...
if menu == "产量预测":
    ...
    for i, feat in enumerate(feature_list):
        with cols[i % 4]:
            input_values[feat] = st.number_input(feat, min_value=0.0, value=default_values[i], step=0.1)
    ...
    if "data" in st.session_state:
        ...
        shap.force_plot(base_value=d['base'], shap_values=d['shap'], features=d['input'].iloc[0].values,
            feature_names=d['en_feats'], matplotlib=True, figsize=(30, 3))
        st.pyplot(plt.gcf())
        ...
        with col_left:
            ...
            show_df = pd.DataFrame({"特征": d['table_feats'], "数值": d['input'].iloc[0].values})
            st.dataframe(show_df, use_container_width=True, height=500)
            st.markdown("</div>", unsafe_allow_html=True)

        with col_right:
            st.markdown("<div class='card'><div class='section-title'>特征贡献度排序</div>", unsafe_allow_html=True)
            imp_df = pd.DataFrame({"特征": d['table_feats'], "贡献值": d['shap']})
